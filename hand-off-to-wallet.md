...
The rail has **only two rows**, both derived from the From wallet's workspace:
...
- `Vaults` — wallets in the **same workspace** as the From wallet.
- `Address Book` — wallets in **any other workspace**.
- **No `All` row.** No `Favorites` row. No divider.

The category set is fixed in size (always exactly two), but the *membership* of each category depends on what the From wallet is. This is the "Mode B — Contextual" pattern from the original two-pane-picker spec.
...
|---|---|
| Fresh open | `Vaults` |
| Re-opening with an existing selection | The rail that selection currently falls into (Vaults if same workspace as From, else Address Book) |
...
- Inside both `Vaults` and `Address Book`, starred wallets get the in-scope favorites grouping: a `Favorites` heading at the top, the starred wallets below, a separator, then the remaining unstarred wallets. Same rule as the From picker for in-category favorites.
...
`Vaults` and `Address Book` rows show **no count badges**. Unlike `All` and `Favorites` in the From picker (which always show live counts), category rows never get counts — and in To mode there are *only* category rows.
...
- [ ] Rail has exactly two rows: `Vaults` and `Address Book`. No meta rows, no divider.
- [ ] `Vaults` = same workspace as From wallet. `Address Book` = any other workspace.
- [ ] No counts on either rail row.
- [ ] Default rail on fresh open: `Vaults`. Re-opening with a selection: the rail that selection falls into.
- [ ] Stars + in-scope Favorites grouping still work inside both Vaults and Address Book — only the `Favorites` rail row is removed.
